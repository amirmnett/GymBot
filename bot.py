...
# ----------------------------------------------------
# ۱. وب‌سرور Flask جهت زنده نگه‌داشتن ربات در Render
# ----------------------------------------------------
web_app = Flask(__name__)

@web_app.route('/')
def home():
    return "🚀 Ultimate Gym Assistant Bot is Active & Running!"
...
# ----------------------------------------------------
# ۲. پیکربندی متغیرهای محیطی و اتصال به Supabase
# ----------------------------------------------------
...
# ----------------------------------------------------
# ۳. ابزارها و توابع کمکی (Visual Tools & Calculators)
# ----------------------------------------------------
...
def calculate_1rm(weight: float, reps: int) -> float:
    """محاسبه یک تکرار بیشینه (Epley Formula)"""
    if reps == 1:
        return weight
    return round(weight * (1 + reps / 30.0), 1)

def get_user_rank(points: int) -> tuple[str, str]:
    """دریافت رتبه و سطح کاربر بر اساس امتیاز"""
    if points < 50:
        return "🌱 تازه‌کار", "سطح ۱"
    elif points < 150:
        return "🥉 ورزشکار مبتدی", "سطح ۲"
    elif points < 300:
        return "🥈 ورزشکار نیمه‌حرفه‌ای", "سطح ۳"
    elif points < 600:
        return "🥇 ورزشکار حرفه‌ای", "سطح ۴"
    elif points < 1000:
        return "💎 قهرمان باشگاه", "سطح ۵"
    else:
        return "⚡ غول باشگاه (Legend)", "سطح نهایی"

# ----------------------------------------------------
# ۴. کیبوردهای اصلی ربات
# ----------------------------------------------------
MAIN_KEYBOARD = ReplyKeyboardMarkup(
    [
        ...
        [KeyboardButton("👤 پروفایل هوشمند من"), KeyboardButton("📊 ثبت سایز و وزن جدید")],
        [KeyboardButton("🏆 جدول رده‌بندی قهرمانان")]
    ],
...
# ----------------------------------------------------
# ۵. فرآیند ثبت‌نام و آنبوردینگ (Onboarding)
# ----------------------------------------------------
    ...
    try:
        ...
        if not res.data or not res.data[0].get("onboarding_completed"):
            ...
            welcome_text = (
                ...
                "╔════════════════════╗\n"
                "║ ⚙️ **تنظیمات اولیه پروفایل** ║\n"
                "╚════════════════════╝\n\n"
                "برای ساخت آنالیز دقیق بدنی و شخصی‌سازی برنامه‌ات، به چند سوال کوتاه پاسخ بده.\n\n"
                "📌 **سوال ۱ از ۷:** چند سال داری؟ (مثال: `24`)"
            )
            await update.message.reply_text(welcome_text, parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
            return OB_AGE
        else:
            await update.message.reply_text(
                f"⚡ **خوش برگشتی قهرمان ({user.first_name})!**\n"
                "آماده‌ای برای یک تمرین پرانرژی؟ از منوی زیر بخش مورد نظرت رو انتخاب کن 👇",
                parse_mode="Markdown",
        ...
        await update.message.reply_text("❌ خطا در اتصال به سیستم. لطفاً لحظاتی بعد مجدداً تلاش کنید.")
        return ConversationHandler.END
...
async def ob_height(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text == "🔙 مرحله قبلی":
        await update.message.reply_text("📌 **سوال ۱ از ۷:** چند سال داری؟", reply_markup=CANCEL_KEYBOARD)
        return OB_AGE
    try:
...
async def ob_weight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return OB_WEIGHT
    await update.message.reply_text("💪 **سوال ۴ از ۷:** اندازه دور بازو (cm) — (اگر نمی‌دانی عدد `0` بفرست):", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return OB_ARM
...
async def ob_arm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🩺 **سوال ۵ از ۷:** اندازه دور سینه (cm) — (یا عدد `0`):", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return OB_CHEST

async def ob_chest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "❌ انصراف": return await cancel(update, context)
    if update.message.text == "🔙 مرحله قبلی":
        await update.message.reply_text("💪 **سوال ۴ از ۷:** اندازه دور بازو (cm):", reply_markup=BACK_OR_CANCEL_KEYBOARD)
        return OB_ARM
    try:
        context.user_data["chest"] = float(update.message.text.strip())
    except ValueError:
        context.user_data["chest"] = 0.0
    await update.message.reply_text("📐 **سوال ۶ از ۷:** اندازه دور کمر (cm) — (یا عدد `0`):", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return OB_WAIST

async def ob_waist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "❌ انصراف": return await cancel(update, context)
    if update.message.text == "🔙 مرحله قبلی":
        await update.message.reply_text("🩺 **سوال ۵ از ۷:** اندازه دور سینه (cm):", reply_markup=BACK_OR_CANCEL_KEYBOARD)
        return OB_CHEST
    try:
...
async def ob_goal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "❌ انصراف": return await cancel(update, context)
    if update.message.text == "🔙 مرحله قبلی":
        await update.message.reply_text("📐 **سوال ۶ از ۷:** اندازه دور کمر (cm):", reply_markup=BACK_OR_CANCEL_KEYBOARD)
        return OB_WAIST
    ...
    await update.message.reply_text(
        "🎉 **پروفایل ورزشی شما با موفقیت ثبت شد!**\n\n"
        "حالا می‌توانی برنامه‌ت رو ثبت کنی یا اولین تمرینت رو شروع کنی. 💪",
        parse_mode="Markdown", reply_markup=MAIN_KEYBOARD
    )
    return ConversationHandler.END

# ----------------------------------------------------
# ۶. ثبت و ویرایش برنامه تمرینی
# ----------------------------------------------------
async def start_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🗓 **چند روز در هفته برنامه تمرینی داری؟**\n(عددی بین ۱ تا ۷ وارد کن):",
...
async def plan_day_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔢 **روز '{day_name}' چند حرکت دارد؟** (مثلاً: `5`)", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
...
async def plan_ex_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cnt.isdigit():
        await update.message.reply_text("⚠️ لطفاً یک عدد معتبر بفرست.", reply_markup=CANCEL_KEYBOARD)
        return PLAN_EX_COUNT
...
async def plan_ex_superset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "بله" in text:
        ...
        await update.message.reply_text("🔗 نام حرکت دوم سوپرست را وارد کن:", reply_markup=CANCEL_KEYBOARD)
...
async def save_exercise_and_next(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if curr_ex < tot_ex:
        ...
        if curr_day < tot_days:
            context.user_data["current_day_idx"] += 1
            await update.message.reply_text(f"✅ تمام حرکات این روز ثبت گردید!\n\n📌 **عنوان روز {curr_day + 1}:** نام روز بعدی چیست؟", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
            return PLAN_DAY_NAME
        else:
            await update.message.reply_text("🔥 **برنامه تمرینی شما با موفقیت و به‌طور کامل ذخیره شد!**", parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)
            return ConversationHandler.END

# ----------------------------------------------------
# ۷. دستیار حرفه‌ای و هوشمند تمرین (Workout Engine)
# ----------------------------------------------------
async def start_gym_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "╔════════════════════╗\n"
            "║  🏋️‍♂️ **انتخاب روز تمرینی** ║\n"
            "╚════════════════════╝\n\n"
            "امروز برنامه کدام روز را می‌خواهی اجرا کنی؟",
...
async def handle_gym_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("start_day_"):
        ...
        if not exercises:
            await query.edit_message_text("❌ هیچ حرکتی برای این روز ثبت نشده است.")
            return
        ...
        await query.message.reply_text(
            f"✍️ **ثبت ست شماره {context.user_data.get('gym_current_set', 1)}**\n\n"
            "وزنه (کیلوگرم) و تعداد تکرار را ارسال کن:\n"
        ...
        await query.edit_message_text("🔴 **تمرین لغو شد.** خسته نباشی!", parse_mode="Markdown")
...
async def receive_gym_set_log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text) < 2:
        await update.message.reply_text("⚠️ **فرمت ارسال اشتباه است!** دو عدد با فاصله بفرست (مثال: `80 10`)", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
        return GYM_LOG_WEIGHT_REPS
    ...
    try:
        ...
        await update.message.reply_text("⚠️ لطفاً اعداد معتبر وارد کن (مثال: `80 10`)", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
        return GYM_LOG_WEIGHT_REPS
    ...
    if idx < len(exercises):
        ...
        est_1rm = calculate_1rm(weight, reps)
        try:
        ...
        await update.message.reply_text(
            f"✅ **ست {current_set} ذخیره شد:** `{weight}kg` × `{reps}` تکرار\n"
            f"🔥 **1RM تخمینی شما در این ست:** `{est_1rm} kg`", 
            parse_mode="Markdown"
        )
...
async def start_rest_timer_live(query, context, seconds):
    ...
    while remaining > 0:
        ...
        if remaining > 0:
            try:
                p_bar = make_progress_bar(int((seconds - remaining) / seconds * 100))
                await context.bot.edit_message_text(
                    chat_id=chat_id, 
                    message_id=msg.message_id, 
                    text=f"⏱️ **استراحت باقی‌مانده:** {remaining} ثانیه\n{p_bar}", 
                    parse_mode="Markdown"
                )
            except Exception:
                pass
...
async def show_current_gym_exercise(query, context):
    ...
    if idx >= len(exercises):
        ...
        try:
            ...
            new_pts = pts + 15
            rank, level = get_user_rank(new_pts)

            supabase.table("users").update({"last_workout_date": today, "points": new_pts}).eq("user_id", user_id).execute()

            finish_msg = (
                "🏆 **خسته نباشی قهرمان! تمرین امروز با موفقیت تمام شد.**\n"
                "════════════════════\n"
                "➕ **۱۵+ امتیاز جدید پاداش گرفتی!**\n"
                f"💎 مجموع امتیازات: `{new_pts}`\n"
                f"🎖 رتبه جدید شما: **{rank}** ({level})"
            )
            await query.edit_message_text(finish_msg, parse_mode="Markdown")
        except Exception as e:
            logger.error(f"Error completing gym session: {e}")
        return

    ex = exercises[idx]
    progress_pct = int((idx / len(exercises)) * 100)
    progress_bar = make_progress_bar(progress_pct)
    
    last_log_txt = "هنوز سابقی ثبت نشده"
    try:
    ...
    txt = f"📊 **پیشرفت جلسه تمرینی:** {progress_bar}\n"
    txt += "════════════════════\n"
    txt += f"🏋️‍♂️ **حرکت {idx + 1} از {len(exercises)}:** {ex['exercise_name']}\n"
    txt += f"🔢 **برنامه:** `{ex['reps']}`\n"
    txt += f"📍 **وضعیت فعلی:** ست شماره {current_set}\n"
    txt += f"📊 **آخرین رکورد ثبت‌شده:** {last_log_txt}\n"
...
# ----------------------------------------------------
# ۸. شبکه اجتماعی ورزشکاران (Social Platform)
# ----------------------------------------------------
async def social_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "════════════════════\n"
        "در این بخش می‌توانی تصاویر و انگیزه تمرینی خودت رو به اشتراک بگذاری، پست‌های بقیه رو لایک کنی و نظرت رو بنویسی!",
        parse_mode="Markdown",
...
async def show_feed_post(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    try:
        ...
        if comments_res:
            ...
            comments_txt += "هنوز نظری برای این پست ثبت نشده است.\n"
...
async def start_add_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("✍️ **نظر یا دیدگاهت رو ارسال کن:**", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
...
async def start_create_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("📷 **لطفاً تصویر تمرین یا انگیزشیت رو ارسال کن:**", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
    return POST_PHOTO

async def receive_post_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "❌ انصراف / بازگشت به منوی اصلی": return await cancel(update, context)
    if not update.message.photo:
        await update.message.reply_text("⚠️ لطفاً یک تصویر معتبر بفرست:", reply_markup=CANCEL_KEYBOARD)
        return POST_PHOTO
        
    photo_file = update.message.photo[-1].file_id
    context.user_data['post_photo'] = photo_file
    await update.message.reply_text("✍️ **متن یا کپشن دلخواهت رو وارد کن:**", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
    return POST_CAPTION
...
async def receive_post_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("🎉 **پست شما با موفقیت در شبکه اجتماعی منتشر شد!**", parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)
    except Exception as e:
...
# ----------------------------------------------------
# ۹. پروفایل هوشمند و آنالیز بدنی
# ----------------------------------------------------
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not res.data:
            await update.message.reply_text("⚠️ پروفایلی برای شما یافت نشد.")
            return
        
        u = res.data[0]
        points = u.get('points', 0)
        rank, level = get_user_rank(points)
        ...
        # محاسبه BMR (کالری پایه)
        bmr_txt = "-"
        if h_m > 0 and w > 0:
            bmr = round(10 * w + 6.25 * (h_m * 100) - 5 * age + 5)
            bmr_txt = f"`{bmr} کیلوکالری/روز`"

        txt = f"👤 **پروفایل هوشمند {u['full_name']}**\n"
        txt += "════════════════════\n"
        txt += f"🎖 **رتبه باشگاه:** {rank} ({level})\n"
        txt += f"💎 **امتیاز کل:** `{points}` امتیاز\n\n"
        txt += f"📏 **قد:** `{u.get('height', '-')}` cm\n"
        txt += f"⚖️ **وزن:** `{u.get('weight', '-')}` kg\n"
        txt += f"📊 **شاخص BMI:** {bmi_txt}\n"
        txt += f"🔥 **سوخت‌وساز پایه (BMR):** {bmr_txt}\n"
        txt += f"💪 **دور بازو:** `{u.get('arm_size', '-')}` cm\n"
        txt += f"🩺 **دور سینه:** `{u.get('chest_size', '-')}` cm\n"
        txt += f"📐 **دور کمر:** `{u.get('waist_size', '-')}` cm\n"
        txt += f"🎯 **هدف اصلی:** {u.get('target_goal', '-')}"
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        msg = "🏆 **جدول برترین قهرمانان باشگاه:**\n"
        msg += "════════════════════\n\n"
        medals = ["🥇", "🥈", "🥉"]
        for idx, row in enumerate(res.data, 1):
            pts = row.get('points') or 0
            rank_title, _ = get_user_rank(pts)
            m = medals[idx-1] if idx <= 3 else f"`{idx}.`"
            msg += f"{m} **{row['full_name']}** — `{pts}` امتیاز | {rank_title}\n"
        await update.message.reply_text(msg, parse_mode="Markdown")
    except Exception as e:
        logger.error(f"Error leaderboard: {e}")

# ----------------------------------------------------
# ۱۰. به‌روزرسانی مشخصات بدنی
# ----------------------------------------------------
async def start_body_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚖️ **وزن جدیدت (کیلوگرم) را بفرست:**", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
    return BODY_WEIGHT
...
async def body_weight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return BODY_WEIGHT
    await update.message.reply_text("💪 **دور بازوی جدید (cm):**", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return BODY_ARM
...
async def body_arm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🩺 **دور سینه‌ی جدید (cm):**", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return BODY_CHEST
...
async def body_chest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📐 **دور کمر جدید (cm):**", parse_mode="Markdown", reply_markup=BACK_OR_CANCEL_KEYBOARD)
    return BODY_WAIST
...
async def body_waist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📊 **سایز و مشخصات جدیدت با موفقیت آپدیت شد!**", parse_mode="Markdown", reply_markup=MAIN_KEYBOARD)
    return ConversationHandler.END
...
# ----------------------------------------------------
# ۱۱. سیستم انگیزش و یادآور عدم فعالیت
# ----------------------------------------------------
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=f"سلام {u['full_name']} عزیز! 🏋️‍♂️\n\n"
                             "۳ روز است که هیچ تمرینی در سیستم ثبت نکرده‌ای! بی‌خیال اهدافت نشو؛ همین امروز با قدرت شروع کن 🔥"
                    )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("^👤 پروفایل هوشمند من$"), show_profile))
    app.add_handler(MessageHandler(filters.Regex("^🏆 جدول رده‌بندی قهرمانان$"), leaderboard))
